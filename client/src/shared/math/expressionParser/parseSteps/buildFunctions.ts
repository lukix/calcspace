...
const buildFunctions = (tokenList) => {
  ...
  const {
    ...
    tokensList,
  } = tokenListWithResolvedSubexpressions.reduce((acc, currentToken, index) => {
    ...
    const hasEncounteredNextArgument =
      currentToken.type === tokens.SUBEXPRESSION && acc.currentFunction;
    ...
    if (
      ...
      acc.previousToken?.type === tokens.SUBEXPRESSION
    ) {
